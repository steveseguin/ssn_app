...
## VK sign-in follow-up (2026-09-08)

- Reproduced the secondary VK ID window displaying Error / Invalid URL. Its POST to https://login.vk.ru/ lacked an Origin header; VK returned error_code: wrong_host.
- The app-wide disable-web-security=false startup flag is preserved. The reusable per-source signin.fillMissingOrigin config rule currently enables tracking only for VK Video Live sign-in windows and their child popups. Only missing Origin headers on POST requests from an id.vk.ru or id.vk.com document to login.vk.ru or login.vk.com are supplied, using the document's actual origin. Existing Origin headers and other source windows are untouched. Tracking ends when each window is destroyed.
- Added vkvideo.signin URL and platform-specific desktop user agent in all three Social Stream settings/config*.json files. The previous fallback stripped live. and opened the separate regular-video site, which redirected to its mobile page with the generic Chrome user agent. No preload, CSP, referrer or popup-policy overrides are needed.
- Existing config header overrides are fixed values attached to the parent window, so they cannot supply the correct document-dependent header to the secondary popup. The new per-source signin.fillMissingOrigin rule supports any source with explicit page origins, request origins, methods and popup scope; see [source sign-in config](source-signin-config.md). This requires the SSApp rule support as well as source configuration; publishing config alone does not fix older builds.
- Real Windows Electron tests use isolated profiles. The phone/email form opens and reopens in the secondary popup, with the same session as the parent. No credentials are submitted, so successful authentication and the return of authenticated cookies remain unverified. Reopening the parent through its native Keep Session prompt, macOS/Linux execution, and new live VK chat capture remain unverified.
- Run node tests/electron/vk-signin-e2e.js. It checks that the global flag remains present, the login request has the expected Origin, VK does not return wrong_host, and the phone form stays visible for ten seconds on each of two popup openings.
- Config-driven test artifacts: %TEMP%/ssapp-vk-signin-R0eSlR (VK), %TEMP%/ssapp-vk-signin-tjB9IA (another source key), and %TEMP%/ssapp-vk-signin-YvJ4h5 (rule absent).
