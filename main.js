                ...
                try {
                    ...
                    if (enforceCloseOnNavigate) {
                        ...
                        view.webContents.on('did-navigate-in-page', (event, url, isMainFrame) => {
                            if (isMainFrame !== false) maybeClose(url, 'did-navigate-in-page');
                        });
                        view.webContents.on('did-redirect-navigation', (details, url, _isInPlace, isMainFrame) => {
                            const redirectUrl = details && typeof details.url === 'string' ? details.url : url;
                            const redirectIsMainFrame = details && typeof details.isMainFrame === 'boolean'
                                ? details.isMainFrame
                                : isMainFrame;
                            if (redirectIsMainFrame !== false) maybeClose(redirectUrl, 'redirect');
                        });
                    }
                } catch (e) {
