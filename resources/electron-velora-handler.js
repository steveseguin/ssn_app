...
const CALLBACK_PATH = "/sources/websocket/velora.html";
const RESULT_ENDPOINT = "/__velora_oauth_result__";
const DEFAULT_AUTH_BASE = "https://auth.socialstream.ninja/auth/velora";
const DEFAULT_TIMEOUT_MS = 5 * 60 * 1000;
const AUTH_MESSAGE_SUCCESS = "ssn-velora-auth-success";
const AUTH_MESSAGE_ERROR = "ssn-velora-auth-error";
const AUTH_RESULT_KEY = "velora_auth_result";
const AUTH_ERROR_KEY = "velora_auth_error";
...
function buildVeloraStartUrl({ authBase, returnTo, origin }) {
...
function createVeloraAuthErrorPayload(message) {
    return {
        type: AUTH_MESSAGE_ERROR,
        message: String(message || "Velora sign-in failed.")
    };
}

function extractVeloraAuthPayload(payload = {}) {
    ...
    if (payload && (payload.queryError || payload.errorDescription)) {
        return createVeloraAuthErrorPayload(payload.errorDescription || payload.queryError);
    }
    return null;
}
...
<html>
<head>
    <title>Velora Authorization</title>
    <style>
        ...
        (function () {
            ...
            fetch('${RESULT_ENDPOINT}', {
                ...
                status.textContent = hasSuccess
                    ...
                    : 'Velora sign-in did not complete.';
...
async function tryStartServer(server) {
    for (const port of LOOPBACK_PORTS) {
        try {
            ...
            console.log(`[Velora OAuth] Server started on port ${port}`);
            return port;
        } catch (error) {
            if (error && error.code === "EADDRINUSE") {
                console.log(`[Velora OAuth] Port ${port} in use, trying next...`);
                continue;
            }
            throw error;
        }
...
function runVeloraLoopbackOAuthSession(payload = {}) {
    return new Promise((resolve, reject) => {
        let timeoutId = null;
        let settled = false;
        let server = null;
        ...
        server = http.createServer((req, res) => {
            ...
            if (req.method === "POST" && parsed.pathname === RESULT_ENDPOINT) {
                ...
                req.on("end", () => {
                    try {
                        ...
                        const authPayload = extractVeloraAuthPayload(incoming);
                        res.writeHead(200, { "Content-Type": "application/json" });
                        res.end(JSON.stringify({ ok: true }));
                        if (!authPayload || typeof authPayload !== "object") {
                            fail(new Error("Velora sign-in returned an unexpected response."));
                            return;
                        }
            ...
            if (parsed.pathname === CALLBACK_PATH || parsed.pathname === "/callback" || parsed.pathname === "/") {
                if (query.error) {
                    const authPayload = createVeloraAuthErrorPayload(query.error_description || query.error);
                    res.writeHead(200, { "Content-Type": "text/html" });
                    res.end(`<!DOCTYPE html>
<html><head><title>Velora Authorization Failed</title>
...
<script>setTimeout(function(){ window.close(); }, 3000);</script></body></html>`);
        ...
        (async () => {
            try {
                ...
                const authUrl = buildVeloraStartUrl({
                ...
                try {
                    ...
                    console.log("[Velora OAuth] Opening auth URL in default browser");
                } catch (shellError) {
                    console.error("[Velora OAuth] Failed to launch default browser:", shellError);
                    fail(shellError);
                }
            } catch (error) {
...
function setupVeloraOAuthHandler() {
    if (ipcMain.listenerCount("velora-oauth") > 0) {
        return;
    }
    ipcMain.handle("velora-oauth", async (_event, payload = {}) => {
        if (activeSession && typeof activeSession.fail === "function") {
            console.warn("[Velora OAuth] Aborting previous pending session in favor of the new request.");
            activeSession.fail(new Error("Previous Velora authentication was interrupted by a new request."));
        }
        return runVeloraLoopbackOAuthSession(payload);
    });
}
