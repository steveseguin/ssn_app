...
async function startMcpBeforeApp(port) {
	...
	try {
		...
		assert.match(unavailable.result.content[0].text, /SSApp.*not reachable/i);
		assert.doesNotMatch(unavailable.result.content[0].text, /ECONNREFUSED|ECONNRESET|connect\s+127\.0\.0\.1/i);
		return session;
	} catch (error) {
		await session.close();
		throw error;
